...
std::string ScreenCaptureWindows::initialize() {
	...
	if (FAILED(hr))
		return "D3D11CreateDevice failed: " + hr;
	...
	hr = dxgiAdapter->EnumOutputs(this->screenNumber, &dxgiOutput);
	dxgiAdapter->Release();
	dxgiAdapter = nullptr;
	if (FAILED(hr)) {
...
bool ScreenCaptureWindows::captureNext() {
	...
	if (SUCCEEDED(hr)) {
		if (Latest.Width != desc.Width || Latest.Height != desc.Height) {
			...
			//Latest.Buf = (BYTE*)malloc(4 * desc.Width * desc.Height);
		}
		for (int y = 0; y < (int)desc.Height; y++)
			memcpy(Latest.Buf.data() + y * desc.Width * 4, (uint8_t*)sr.pData + sr.RowPitch * y, desc.Width * 4);
			//memcpy(Latest.Buf + y * desc.Width * 4, (uint8_t*)sr.pData + sr.RowPitch * y, desc.Width * 4);
